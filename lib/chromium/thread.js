...
var ChromiumThreadActor = ActorClass({
  ...
  onPaused: task.async(function*(params) {
    if (this.state === "running" && this.pauseOutstanding) {
      this.pauseOutstanding = false;

      yield this.updateExceptionState();
  ...
  updateExceptionState: task.async(function*() {
    let state = this.getDesiredExceptionState();
