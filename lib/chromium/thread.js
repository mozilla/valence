...
var ChromiumThreadActor = ActorClass({
  ...
  onPaused: task.async(function*(params) {
    ...
    if (this.pseudoPaused && this.pseudoResuming) {
      this.pseudoResuming();
    } else if (this.pseudoPaused) {
      // The pseudopause has become a real pause.
  ...
  attach: asyncMethod(function*(options) {
    ...
    this.pseudoResumeScriptId = response.scriptId;
  ...
  pseudoPause: task.async(function*() {
  ...
  resume: asyncMethod(function*(options) {
    ...
    if (options.pauseOnExceptions) {
      exceptionState = options.ignoreCaughtException ? "uncaught" : "all";
    }

    console.log("Promising resumption");
    yield new Promise(task.async(function*(resolve, reject) {
      console.log("Starting resume...");
      finishResuming = task.async(function*() {
        console.log("Finishing resume");
        this.pseudoPaused = false;
        this.pseudoResuming = false;
        yield this.rpc.request("Debugger.setPauseOnExceptions", {
          state: exceptionState
        })
        this.stopPause();
        this.rpc.request(resumeCommand).then(resolve, reject);
      }.bind(this));

      if (!this.pseudoPaused) {
        finishResuming();
        return;
      }

      // We are pseudoPaused, which means we've asked for a pause but
      // it hasn't actually happened yet.  Kick an exception to try to get
      // things to pause.
      this.pseudoResuming = finishResuming;
      ...
      yield this.rpc.request("Debugger.runScript", {
        scriptId: this.pseudoResumeScriptId,
