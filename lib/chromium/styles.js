...
var ChromiumPageStyleActor = protocol.ActorClass({
   ...
   */
  setPropertyText: task.async(function*(rule, prop, newName, newValue) {
    let range;
    if (prop) {
      range = prop.range;
    } else {
    ...
    let response = yield this.rpc.request("CSS.setPropertyText", {
      styleSheetId: rule.style.styleSheetId,
      range: range,
      text: newName ? newName + ":" + newValue + ";" : ""
    });
    ...
    let newRange;
    let newName = newName || prop.name;
    for (let newProp of rule.style.cssProperties) {
      if (newProp.name === newName && newProp.range) {
...
var ChromiumStyleRuleActor = protocol.ActorClass({
  ...
  modifyProperty: task.async(function*(mod) {
    ...
    if (mod.type === "set") {
      ...
      yield this.pageStyle.setPropertyText(this, foundProp, mod.name, mod.value);
