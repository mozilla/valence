...
// "startGroupCollapsed" , "table" , "timing" , "trace" ] ) 
  ...
  autocomplete: asyncMethod(function*(text, cursor) {
    ...
    let { result: thisv } = yield this.rpc.request("Runtime.evaluate", {
      expression: "this"
    });

    let { result: propDescArray } = yield this.rpc.request("Runtime.getProperties", {
      objectId: thisv.objectId,
      ownProperties: true
    });

    let result = JSPropertyProvider(propDescArray, text, cursor) || {};
    let matches = result.matches || [];
    let reqText = text.substr(0, cursor);

    // XXX: add command line API
