...
let iOSProxy = {
  ...
  checkIfBound: task.async(function*() {
    ...
    if (Runtime.OS == "WINNT") {
      return this.runInShell('PowerShell -Command "& {netstat -na -p tcp|Select-String -simpleMatch :9230|Select-String -caseSensitive -list -simpleMatch LISTENING}"');
    }
   ...
   *        The arguments array to pass to the command.
   */
  runInShell: task.async(function*(command, args = []) {
